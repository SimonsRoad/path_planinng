...
if n > 1000, error('B?ier curve for more than 1000 points is not allowed'); end
...
if ~isempty(intcurvexx)    % The user wants interpolation curve
  intcurveyy = interp1(bezcurve(:, 1), bezcurve(:, 2), intcurvexx, 'PCHIP');  % Makes cubic interpolation from the Bezier curve. Other options: 'linear' or 'spline'
end
