function [check,shp_obj]=is_in_blind3(target1,target2,FOV,x_pnts)
...
%     x_pnts = reshape(x_pnts,[],3);
    ...
    for i = 1:N              
        ...
        x = r*cos(theta_i(i));        
        for j = 1:M
            y = l_i*cos(theta_j(j)) + r*sin(theta);
            z = l_i*sin(theta_j(j));
            pcl = [pcl  ; [x y z]];
        end
    end
