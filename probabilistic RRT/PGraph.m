...
%             g.ncvalid = false;  % mark connectivity as suspect
        ...
        function d = distance(g, v1, v2,metric)
            ...
            % currently vertex information: v(1:2) xy coordinate /
            % v(3) : colision probability 
             
            
            switch metric
                case 'distance'
                    d=norm(g.vertexlist(1:2,v1)-g.vertexlist(1:2,v2));
                case 'probability'
                    d=log(1-g.vertexlist(3,v2)); % sign: minus 
        ...
        function [c,dn] = closest(g, p,metric ,tol)
...
%                        fprintf('egde was %f',e) 
        ...
        function [path,cost] = Astar(g, vstart, vgoal,metric,varargin)
            ...
            h_score(vstart) = g.distance(vstart, vgoal,metric);
            ...
            while ~isempty(openSet)
                ...
                for k=1:length(neighbours)
                    ...
                    if ~ismember(neighbour, openSet)
                        ...
                        h_score(neighbour) = g.distance(neighbour, vgoal,metric);
...
%                         end
        ...
        function d = distance_metric(g, x1, x2,metric)
            global S
