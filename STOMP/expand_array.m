            ...
            if (k~=j || k~=0)  %The node itself is not its successor
                ...
                if( (s_x >0 && s_x <=MAX_X) && (s_y >0 && s_y <=MAX_Y))%node within array bound
                    ...
                    if (flag == 1)
                        ...
                        exp_array(exp_count,3) = hn+distance_Astar(node_x,node_y,s_x,s_y);%cost of travelling to node
                        exp_array(exp_count,4) = distance_Astar(xTarget,yTarget,s_x,s_y);%distance between node and goal
